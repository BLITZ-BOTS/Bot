...
export interface Command {
  name: string;
  description: string;
  action: (interaction: ChatInputCommandInteraction, config: Record<string, unknown>) => Promise<void>;
}

export interface Event {
  event: string;
  once?: boolean;
  action: (client: Client, config: Record<string, unknown>, ...args: unknown[]) => Promise<void>;
}
