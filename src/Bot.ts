import { Client, Collection, IntentsBitField, SlashCommandBuilder } from "discord.js";
...
export class Bot {
    ...
    private async loadPlugins() {
        ...
        console.info(`Loaded ${this.plugins.length} plugins with ${this.commands.size} commands`);
    }

    private async registerCommands() {
        if (!this.client.user) {
            console.error("Client user is not available. Commands registration aborted.");
            return;
        }
        ...
        try {
            await rest.put(
                Routes.applicationGuildCommands(this.client.user.id, "1287858702430376039"),
                { body: commands }
            );
